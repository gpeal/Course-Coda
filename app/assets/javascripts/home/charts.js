...
function loadChartData(data, xRange, yRange) {
  for(var i = 0; i < data.length; i++) {
    var organizedSection = organizedSections[sectionName(data[i])];
    if(organizedSection === undefined) {
      organizedSections[sectionName(data[i])] = []
    }
  ...
  var terms = generateTerms(xRange.firstQuarter.title, parseInt(xRange.firstYear.title));
  // draw the charts
